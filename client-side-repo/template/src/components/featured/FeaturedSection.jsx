...
const FeaturedSection = () => {
 ...
 {loading&&<span className="loading loading-spinner loading-lg text-center "></span>
    ...
    <div className=" grid grid-cols-1 md:grid-cols-2 gap-5 w-[98%] mx-auto ">
      ...
      {
        data?.map(singleData=><div key={singleData._id} className="card  card-side bg-base-300 shadow-xl flex-col lg:flex-row-reverse p-2  border border-yellow-300">
      ...
      <Link to={'/assignments'}>
      <button className=" btn-square"><FaArrowCircleRight className=" text-3xl font-bold text-purple-400"></FaArrowCircleRight></button>
</Link>
        </div>
      </div>)
      }
